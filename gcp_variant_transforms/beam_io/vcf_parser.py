...
class VariantCall(object):
  ...
  def __eq__(self, other):
    return (
        (self.name, self.genotype, self.phaseset, self.info, self.sample_id) ==
...
class PyVcfParser(VcfParser):
  ...
  def _get_variant_calls(self, record, formats):
    ...
    for sample in record.samples:
      ...
      if self._file_hash:
        call.sample_id = generate_int64_hash_code(self._file_hash, call.name)
...
def generate_int64_hash_code(file_hash, sample_name):
  ...
  to signed int.
  """
  return cityhash.CityHash64(file_hash + sample_name) - sys.maxint - 1
