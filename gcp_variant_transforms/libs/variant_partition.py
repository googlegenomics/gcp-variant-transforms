...
"""Encapsulates all partitioning logic used by VariantTransform pipeline.
...
_DEFAULT_NUM_PARTITIONS = _RESERVED_PARTITIONS + 5
...
class RegionIndexer(object):
  """To assign indices to multiple regions or one single whole chromosome."""
  def __init__(self):
    # At this stage, this instance can contain either role.
    self._index = -1
...
class VariantPartition(object):
  ...
  def _get_config_partition(self, reference_name, pos):
    # Look up _by_ref_name dict to see if we have an entry for this ref_name.
  ...
  def _get_auto_partition(self, reference_name):
    ...
    Given a reference_name returns an index in [0, _DEFAULT_NUM_PARTITIONS) range.
    ...
    if partition is None:
      ...
      remaining_partitions = _DEFAULT_NUM_PARTITIONS - _RESERVED_PARTITIONS
