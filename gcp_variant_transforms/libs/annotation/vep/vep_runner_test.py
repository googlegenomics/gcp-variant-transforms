...
class VepRunnerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._mock_jobs = mock.MagicMock()
        self._mocked_batch_api = MockedBatchAPI(self._mock_request)
        ...
        self._mock_locations.jobs = mock.MagicMock(return_value=self._mocked_batch_api)
    ...
    def test_make_vep_cache_path(self):
        ...
        test_instance = vep_runner.VepRunner(
            ...
            "",
            _NUM_FORK,
            self._get_pipeline_args(),
            None,
            30,
        )
        self.assertEqual(
            test_instance._vep_cache_path,
            ("gs://variant-data/vep/" "vep_cache_homo_sapiens_GRCh38_104.tar.gz"),
        )
        test_instance = vep_runner.VepRunner(
            self._mock_service,
            _LOCATION,
        ...
        self.assertEqual(
            test_instance._vep_cache_path,
            ("gs://variant-data/vep/" "vep_cache_mus_musculus_GRCm39_104.tar.gz"),
        )
    ...
    def _validate_run_for_all_files(self):
        self._mocked_batch_api.validate_calls([f[0] for f in _INPUT_FILES_WITH_SIZE])

    def test_run_on_all_files(self):
        num_runnables = len(_INPUT_FILES_WITH_SIZE) * _NUM_RUNNABLES
        test_instance = self._create_test_instance(
            self._get_pipeline_args()
        )
        with patch("apache_beam.io.filesystems.FileSystems", _MockFileSystems):
            test_instance.run_on_all_files()
        self.assertEqual(self._mocked_batch_api.num_run_calls(), num_runnables)
        self._validate_run_for_all_files()

    def test_run_on_all_files_with_more_workers(self):
        test_instance = self._create_test_instance(self._get_pipeline_args())
        with patch("apache_beam.io.filesystems.FileSystems", _MockFileSystems):
            test_instance.run_on_all_files()
        self.assertEqual(
            self._mocked_batch_api.num_run_calls(), len(_INPUT_FILES_WITH_SIZE) * _NUM_RUNNABLES
        )
        self._validate_run_for_all_files()
    ...
    def test_wait_until_done(self):
        ...
        mock_locations.jobs = mock.MagicMock(return_value=mock_jobs)
        mock_jobs.get = mock.MagicMock(return_value=mock_request)
        mock_request.execute = mock.MagicMock(return_value={"status": {"state": "SUCCEEDED"}})
        test_instance = self._create_test_instance()
        with patch("apache_beam.io.filesystems.FileSystems", _MockFileSystems):
            test_instance.run_on_all_files()
    ...
    def test_wait_until_done_fail(self):
        ...
        mock_request.execute = mock.MagicMock(
            return_value={"status": {"state": "FAILED"}}
        )
    ...
    def validate_calls(self, input_file_list):
        ...
        for one_call_actions in self._runnables_list:
            ...
            if start_len == len(input_file_set):
                logging.error(
                    "None of input files appeared in %s or it was repeated",
